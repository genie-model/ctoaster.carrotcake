...
platform_script_path = os.path.join(U.ctoaster_root, "platforms", platform)
with open(platform_script_path) as file:
    exec(file.read())
...
class Application(AfterHandler, ttk.Frame):
    def __init__(self, master=None):
        ...
        self.normal_font = tk.font.nametofont("TkDefaultFont")
        self.mono_font = tk.font.nametofont("TkFixedFont")
    ...
    def move_rename(self):
        ...
        d = MoveRenameDialog(full_path, is_folder, self.job_folder.possible_folders())
        if d.result:
    ...
    def run_job(self):
        ...
        with open(os.path.join(self.job.jobdir, "run.log"), "a") as fp:
            try:
                pipe = sp.Popen(
                    runexe, cwd=self.job.jobdir, stdout=fp, stderr=sp.STDOUT
                )
    ...
    def item_selected(self, event=None):
        ...
        if len(self.tree.get_children(jobid)) != 0 or self.job_folder.is_folder(jobid):
            self.job = None
        else:
        ...
        self.state_buttons = {
            ...
            "COMPLETE": ["move_rename", "clear_job", "delete_job", "clone_job"],
            "ERRORED": ["move_rename", "clear_job", "delete_job", "clone_job"],
        }
    ...
    def set_job_buttons(self):
        ...
        if self.job == None:
            ...
            # the folder.
            for k, v in self.tool_buttons.items():  # Changed .iteritems() to .items()
                if k in self.switchable_buttons:
                    e = (k == "move_rename" or k == "delete_job") and self.tree.parent(
                        sel
                    ) != ""
                    enable(v, e)
        else:
        ...
        if self.job == None:
            ...
            for k, v in self.tool_buttons.items():  # Changed .iteritems() to .items()
                if k in self.switchable_buttons:
                    ...
                    self.job_menu.entryconfig(
                        self.menu_items[k], state=tk.NORMAL if e else tk.DISABLED
                    )
        else:
            # A job is selected: the actions that are enabled depend
            # on the job status.
    ...
    def create_widgets(self):
        ...
        for t, title, dia in tool_info:
            if t == "spacer":
                ...
                img = tk.PhotoImage(
                    file=os.path.join(U.ctoaster_root, "tools", "images", t + ".gif")
                )
                b = ttk.Button(self.toolbar, image=img, command=getattr(self, t))
                b.image = img
                b.pack()
                ToolTip(b, title)
        ...
        self.menu_items = {}
        top["menu"] = self.menu
        self.job_menu = tk.Menu(self.menu, tearoff=0, postcommand=self.set_menu_state)
    ...
    def find_configs(self):
        ...
        self.base_configs = list(
            map(lambda s: s.rpartition(".")[0], bs)
        )  # Convert map to list
