    ...
    try:
        ...
        with open(f) as fp:
            for line in fp:
                ...
                if m:
                    res[m.group(1)] = clean(m.group(2).strip())
            return res
...
def load_module_info():
    try:
        with open(os.path.join(U.ctoaster_root, "src/module-info.csv")) as fp:
        ...
        error_msg = "Couldn't open module info file " + os.path.join(
            srcdir, "module-info.csv"
        )
        if not srcdir:
...
def timestepping_options(runlen, coords, t100, quiet=False):
    ...
    if not quiet:
        if not quiet:
            print(
                f"Setting time-stepping [GOLDSTEIN, BIOGEM:GOLDSTEIN]: {nsteps} {dbio}"
            )
    ...
    res.update(
        {
            k: dbio
            for k in [
                "ma_conv_kocn_katchem",
                "ma_conv_kocn_kbiogem",
                "ma_conv_kocn_krokgem",
            ]
        }
    )
    ...
    for k in ["bg_par_misc_t_runtime", "sg_par_misc_t_runtime"]:
        res[k] = runlen

    # Overall cTOASTER run length.
    for k in ["ma_koverall_total", "ma_dt_write"]:
        res[k] = runlen * 5 * nsteps
...
def restart_options(restart):
    ...
    if restart:
        ...
        res.update(
            {p + "_ctrl_continuing": ".FALSE." for p in ["ac", "bg", "sg", "rg"]}
        )
...
def copy_data_files(m, nml, outdir, extras):
    ...
    def check_data_item(s):
        if not isinstance(s, str):
            return False
        if s.lower() in [".true.", ".false.", "n", "y", m]:
            return False
        if re.match(fp_re, s):
            return False
        if s == "input/" + m:
    ...
    cands = [os.path.basename(f) for f in nml.entries.values() if check_data_item(f)]
